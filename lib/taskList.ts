import type { AnyFunction, TaskKeyType, PriorityTimeoutParams } from '../utils/task'
...
export class TaskList {
  ...
  private map = new Map<TaskKeyType, Node>();
  /**跳表最大层级(小于等于MAX_LEVEL) */
  private maxLevel = 0;
  /**表头 */
  private head = new Node(new Task(HEAD));
  /**表尾 */
  private tail = new Node(new Task(TAIL));
  /**优先级对应超时时间 */
  private priorityTimeout: typeof DEFAULT_PRIORITY_TIMEOUT;
  /**
  ...
  getFirstTimeOut() {
    ...
    if (!firstTask) throw new Error('非空跳表中不存在首个任务')
  ...
  shift() {
    ...
    let currentNode = this.head.getNext(0);
    if (!currentNode) throw new Error('非空跳表中不存在首个任务')

    // 从跳表中删除首个任务
    for (let i = this.maxLevel - 1; i >= 0; i--) {
      let preNode = currentNode.getPre(i);
      let nextNode = currentNode.getNext(i);
      if (!preNode && !nextNode) continue;
      if (!preNode) throw Error(`首个任务，第${i}层级,只存在nextNode,不存在preNode`)
      if (!nextNode) throw Error(`首个任务，第${i}层级,只存在preNode,不存在nextNode`)
  ...
  put(val: AnyFunction, { key = Symbol('default'), priority = PRIORITY_TYPE.NORMAL, callback }: {
    ...
    if (this.has(key)) {
      ...
      // 获取相同key值任务
      let node = this.get(key) as Node;
      // 获取key值对应的任务
      let task = node.getTask();

      // 旧任务重新赋值
      node.getTask().setVal(val);

      // 合并新旧任务回调函数
      if (callback) task.addCallback(callback)
      ...
      if (timeout < task.getTimeout()) {
        ...
        for (let i = level - 1; i >= 0; i--) {
          let pre = node.getPre(i);
          let next = node.getNext(i);
          if (!pre || !next) throw Error(`任务【${key.toString()}】，第${i}层级,不存在nextNode或者preNode`)
          pre.setNext(i, next);
          next.setPre(i, pre);
        }
        // 各层级插入新的位置
        for (let i = level - 1; i >= 0; i--) {
          ...
          if (!preNode) throw Error(`任务:${key.toString()}，第${i}层级,不存在preNode`)

          while (preNode !== this.head && preNode.getTask().getTimeout() > task.getTimeout()) {
            nextNode = nextNode.getPre(i);
            if (!nextNode) throw Error(`任务【${key.toString()}】，第${i}层级,不存在nextNode`)
      ...
      // 不存在key值相同的任务
      // 生成当前任务跳表层级
      let level = TaskList.generateLevel();
      // 创建任务
      let task = new Task(key, val, { callback, priority, timeout })
      ...
      for (let i = level - 1; i >= 0; i--) {
        ...
        if (!nextNode) throw new Error(`任务【${preNode.getTask().getKey().toString()}】,不存在nextNode`)

        while (nextNode !== this.tail && nextNode.getTask().getTimeout() <= task.getTimeout()) {
          let _preNode = preNode.getNext(i)
          if (!_preNode) throw new Error(`不存在preNode`)
          preNode = _preNode;
          nextNode = nextNode.getNext(i);
          if (!nextNode) throw new Error(`不存在nextNode`)
        }
